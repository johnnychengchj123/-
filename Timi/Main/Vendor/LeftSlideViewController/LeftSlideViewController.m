                     ...
                     andMainView:(UIViewController *)mainVC
{
    if(self = [super init]){
        ...
        TMScrollViewController *scrollVc = (TMScrollViewController *)self.mainVC;
        self.leftVC.view.hidden = YES;

        //滑动手势
        self.pan = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(handlePan:)];
        [scrollVc.contentScrollView addGestureRecognizer:self.pan];
...
//        
//        
//        self.rightVC.view.frame = CGRectMake(kScreenWidth, 0, kScreenWidth, kScreenHeight);
//        //设置左侧tableview的初始位置和缩放系数
////        self.rightVC.view.transform = CGAffineTransformMakeScale(kLeftScale, kLeftScale);
////        self.rightVC.view.center = CGPointMake(kLeftCenterX, kScreenHeight * 0.5);
//        
//        [self.view addSubview:self.mainVC.view];
...
- (void) handlePan: (UIPanGestureRecognizer *)rec{
    ...
    if (rec.view.frame.origin.x <= 0 ) {
        [scrollVc.contentScrollView setContentOffset:CGPointMake(kScreenWidth, 0) animated:YES];
    }
    
    if (scrollVc.contentScrollView.contentOffset.x == kScreenWidth) {
        [scrollVc.contentScrollView setContentOffset:CGPointMake(0, 0) animated:YES];
    }
    
//    
//    
//    BOOL needMoveWithTap = YES;  //是否还需要跟随手指移动
//    if ((self.mainVC.view.x <= -kScreenWidth) || ((self.mainVC.view.x >= (kScreenWidth - kMainPageDistance))))
//    {
//        //边界值管控
//        _scalef = 0;
//        needMoveWithTap = NO;
...
//        
////        NSLog(@"rec.view.center.x:%f,point.x %f,self.speedf%f",rec.view.center.x ,point.x ,self.speedf);
//
//
//    
//    
...
//    
//    
//        NSLog(@"rec.view.frame:%f,%f,%f,%f",rec.view.frame.origin.x,rec.view.frame.origin.y,rec.view.frame.size.height,rec.view.frame.size.width);
//        NSLog(@"self.mainVC.view.frame:%f,%f,%f,%f",self.mainVC.view.frame.origin.x,self.mainVC.view.frame.origin.y,self.mainVC.view.frame.size.height,self.mainVC.view.frame.size.width);
//        
//        
//
